...
if __name__ == '__main__':
    ...
    # Read in databases
    referencedb = connect('../databases/reference_lead.db')
    # Get the gas phase energy of CO
    Pbstat = referencedb.get(formula='Pb4', pw_cutoff=500.0, functional='BF')
    Pb_E =  get_vibrational_energy(Pbstat, [], method='novib', geometry='linear', \
            symmetrynumber=1)['E'] / 4
        ...
        for index, state in enumerate(states):
            if state != 'slab':
                ...
                if int(total_rows) > 1:
                    ...
                    ax[int(row), column].set_ylabel(r'$\Delta E_{Pb}^{*}$ \ eV',)
                else:
                    ...
                    ax[column].set_ylabel(r'$\Delta E_{Pb^{*}}$ \ eV',)
