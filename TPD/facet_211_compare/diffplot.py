...
for functional in functionals:
    ...
    states = get_states(thermodb, shorthand_functional[functional], '1x3')
    for state in states:
...
ax2 = ax1.twiny()
color = 'tab:blue'
ax2.set_xlabel(r'$\theta_{experiment}$', color='k') 
ax2.xaxis.set_ticklabels([])
# get Experimental data
data_exp = pickle.load(open('desorption_energy.pickle', 'rb'))
Ed = data_exp['Ed']
theta = data_exp['theta']
ax2.plot(np.flip(theta)/1.8, -1 * np.flip(Ed), color='k', lw=4, label='Experiment')
