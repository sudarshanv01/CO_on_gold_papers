...
temp_range_min = [250, 300] # Temperature range to find minimum rate
temp_range = [170, 270] # Temperature range to plot the TPD in
...
def gaussian(x, a, sigma):
    values = a * np.exp( - (x - 190)**2 / ( 2* sigma**2))
    return values
...
for exposure in exposures:
    ...
    popt, pcov = curve_fit(gaussian, temperature, rate_exp,\
            p0=[rate_max, sigma_rate])
    temper_simu = np.linspace(90, 300)
    writer = csv.writer(f)
    for i in range(len(temper_simu)):
...
for nu in random_nu:
    ...
    for exposure in Ed_exposure:
        ...
        plt.plot(temperature[0:len(Ed_min)], rate_simulated)
        plt.plot(temperature, rates * beta, 'kv')
    plt.ylabel('TPD rates \ ML / s')
    plt.xlabel('Temperature \ K')
    plt.legend(loc='best')
