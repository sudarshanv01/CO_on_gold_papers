...
for key in tpd_data:
    ...
    plt.plot(temperature, rate, 'o', label='exposure = ' + str(key) )
    #if key == max_exposure:
    spline_TPD = get_spline_TPD(temperature, rate, scaling_smoothing=0.0)
    plt.plot(temperature, spline_TPD(temperature), '--', color='gray')
...
"""
DFT_binding_energies = [[1, 0.25], [0.5, 0.33], [0.33, 0.35]]
annotations_DFT = ['1 ML', '1/2 ML', '1/3 ML']
random_nu = np.array([ 10e2, 10e13,])
...
for exposure in exposures:
    ...
    plt.axhline(0.52, color='k', lw=2, ls='--', label='Redhead Analysis from paper')
    plt.xlim([0, 1.1])
    plt.legend(loc='best')
    #plt.ylim([0.4,0.9])
...
for nu in random_nu:
    ...
    fit_Ed_max.set_smoothing_factor(0.) # fit all points
