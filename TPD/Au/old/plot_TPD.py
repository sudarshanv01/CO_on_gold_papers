...
for key in tpd_data:
    ...
    if key == max_exposure:
        spline_TPD = get_spline_TPD(temperature, rate, scaling_smoothing=0.1)
...
"""
data_TPD_largest = get_normalized_TPD(tpd_data[max_exposure][0], tpd_data[max_exposure][1], beta, scaling_smoothing=0.1)
...
for exposure in exposures:
    ...
    if exposure == max_exposure:
        ...
        data_TPD = get_normalized_TPD(temperature, rate,\
                maximum_sites_from_spline=maximum_sites_from_spline, beta=beta, scaling_smoothing=0.4)
...
for exposure in exposures:
    plt.plot(quant_TPD[exposure]['temperature'], \
            quant_TPD[exposure]['coverages'], label='exposure: ' + str(exposure))
plt.ylabel(r'$\theta$')
plt.ylim([0, 1])
plt.xlabel('Temperature \ K')
plt.legend(loc='best')
...
random_nu = np.array([ 10e10, 10e11, 10e12, 10e13])
colors = ['sienna', 'olive', 'navy', 'orchid']
