...
data_TPD_largest = get_normalized_TPD(tpd_data[max_exposure][0], \
        tpd_data[max_exposure][1], beta, scaling_smoothing=0.0)
...
for exposure in exposures:
    plt.plot(quant_TPD[exposure]['temperature'], \
            quant_TPD[exposure]['coverages'], label='coverage: ' + str(exposure))
...
for key in fit_Ed_maxEx_nu:
    ...
    plt.plot(coverage_range, eVtokjoulepmol * fit_Ed_maxEx_nu[key](coverage_range), label=r'$log\nu$ = ' + str(np.log10(key)), color=colors_nu[key])
    # Now plot the exact data points
    #plt.plot(coverage_range, Ed_maxEx_nu[key], 'o')

plt.xlabel(r'$\theta \ ML$')
plt.ylabel('Desorption Energy \ kJ/mol')
plt.legend(loc='best')
plt.xlim([0, 1.2])
