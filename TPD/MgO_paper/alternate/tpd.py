...
temp_range_min = [116, 118] # Temperature range to find minimum rate
temp_range = [100, 118] # Temperature range to plot the TPD in
sat_coverage = 1 # Maximum coverage possible in ML
sat_area = 0.4 # based on the area under curve vs exposure graph
beta = 0.6 # heating rate in K/s
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def ode_rates(theta, T, Ed0, beta, nu):
    import numpy as np
    kB = 8.617e-05 # eV/K
    Ed = Ed0 # assume constant with coverage
    k = nu * np.exp( - Ed / kB / T )
    dtheta_dT = k * theta / beta
    return dtheta_dT

fixed_nu = 10e13
Ed_variable = np.array([0.2, 0.3, 0.4])
colors_Ed = dict(zip(Ed_variable, colors))
plt.figure()
for exposure in exposures:
    rates = ndata[exposure]['rate']
    temperature = ndata[exposure]['temperature']
    theta_exp = ncoverage[exposure]
    plt.plot(temperature, rates, 'kv')
    for Ed in Ed_variable:
        rates_simulated = simulated_rates(theta_exp, fixed_nu, Ed, temperature)
        plt.plot(temperature, rates_simulated / beta, color=colors_Ed[Ed], ls='--')

plt.ylabel('TPD rates \ ML / s')
plt.xlabel('Temperature \ K')
plt.legend(loc='best')
plt.savefig(output + 'simulated_TPD_fixedEd.pdf')
