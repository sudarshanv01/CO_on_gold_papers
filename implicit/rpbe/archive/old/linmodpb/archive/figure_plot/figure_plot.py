...
if __name__ == '__main__':
    ...
    thermodb = connect('Au_211_D3_thermo.db')
...
#    referdb = connect("/Users/vijays/OneDrive - Danmarks Tekniske Universitet/project/tools/references/reference_QE.db")
    ...
    referdb = connect('references_vasp.db')
    ...
    for i in range(len(states)):
        if states[i] != 'slab':
            ...
            #plt.figure()
            psigma = -1*(sigma-9892.21886728)
            plt.plot(psigma, binding_energy_implicit, 'ro--', label=r'$\Delta G_{'+states[i]+'} - implicit$')
            range_sigma = np.arange(min(psigma), max(psigma))
